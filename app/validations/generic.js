...
const phoneCheck = (phoneField, joiObject) => joiObject
  .string()
  .required()
  .pattern(new RegExp('^[0-9]{11,14}$'))
  .messages({
    'string.pattern.base': `${phoneField} must be a number between 11 and 14 digits`,
    'string.empty': `${phoneField} must not be an empty field`,
    'any.required': `${phoneField}  is a required field`
  });

const editPhoneCheck = (phoneField, joiObject) => joiObject
  .string()
  .pattern(new RegExp('^[0-9]{11,14}$'))
  .messages({
    'string.pattern.base': `${phoneField} must be a number between 11 and 14 digits`,
